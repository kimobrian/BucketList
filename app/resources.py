...
class BucketListsAction(Resource):
    ...
    def get(self, bucketlist_id=None):
        ...
        if bucketlist_id is None:
            bucket_lists = BucketList.query.filter_by(created_by=g.user_id).all()
                ...
                for bucket_list in bucket_lists:
                    ...
                    if len(bucket_list_items) > 0:
                        for bucketlist_item in bucket_list_items:
                            ...
                            b_list_item_details['date_created'] = bucketlist_item.date_created
                            b_list_item_details['date_modified'] = bucketlist_item.date_modified
            ...
            try:
                bucket_list= BucketList.query.filter_by(id=bucketlist_id, created_by=g.user_id).one()
                bucketlist_details = {}
                bucketlist_details['id'] = bucket_list.id
                bucketlist_details['name'] = bucket_list.name
                bucketlist_details['items'] = []
    ...
    def post(self):
        ...
        if not name:
            response = jsonify({'message': 'Please provide name for your bucketlist'})
            response.status_code = 400
            return response
        try:
            check_name = BucketList.query.filter_by(name=name, created_by=g.user_id).one()
            ...
            try:
                ...
                response = jsonify({'message': 'Error Occurred Saving Bucketlist'})
                response.status_code = 200
                return response
    ...
    def put(self, bucketlist_id):
        ...
        if not new_data:
            response = jsonify({'message': 'Please provide new Bucket List Name'})
            response.status_code = 200
            return response
        try:
            check_name = BucketList.query.filter_by(name=new_data, created_by=g.user_id).one()
            response = jsonify({'message': 'Name already in use'})
            response.status_code = 200
            return response
        except Exception: # No bucketlist with the name found(Safe update)
            updated_rows = BucketList.query.filter_by(id=bucketlist_id, created_by=g.user_id).update({'name': new_data})
            if updated_rows == 0:
                response = jsonify({'message': 'No bucketlist with that ID'})
                response.status_code = 200
                return response
            elif updated_rows > 0:
                response = jsonify({'message': 'Bucket List updated Successfully'})
                response.status_code = 200
                return response
