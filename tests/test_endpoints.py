...
class EndpointTests(BaseTestSetup):
    ...
    def test_login_with_invalid_details(self):
        ...
        self.assertEqual(response.json, {'message': 'Invalid email, password combination'})
    ...
    def test_login_without_details(self):
        ...
        self.assertEqual(response.json, {'message': 'Provide both username and password'})
        self.assert400(response)


    def test_user_register_with_details(self):
        '''Test user registration when email and password are provided'''
        current_users_count = User.query.count() # User count before registration
        data = {'email': 'kevin123@gmail.com', 'password': 'password123'}
        response = self.app.post('/auth/register/', data=data)
        after_registration_count = User.query.count() # User count after registration
    ...
    def test_user_register_without_details(self):
        ...
        data = {'email': '', 'password': ''}
        response = self.app.post('/auth/register/', data=data)
        self.assertEqual(response.json, {'message': 'Provide both email and password'})
        self.assert400(response)

    def test_registration_for_unique_email(self):
        '''Test for unique registration email'''
        responseB = self.app.post('/auth/register/', data=self.reg_data) #Registration with same email address
        responseB_data = responseB.json
        self.assertEqual(responseB_data, {'message': 'Email address already in use'})
        self.assert400(responseB, message='Email is not in use(Valid Registration)')

    def test_bucket_lists_result_with_non_existent_id(self):
        """Test bucketlists result when the id provided is not available(Single Bucketlist)"""
        response = self.app.get('/bucketlists/43/', headers=self.header_content_token)
        self.assertEquals(response.json, {'message': 'No Bucket List with that ID'})
        self.assert200(response)

    def test_bucket_lists_result_with_existing_id(self):
        """Test bucketlists result when the id provided is available(Single Bucketlist)"""
        response = self.app.get('/bucketlists/1/', headers=self.header_content_token)
        self.assertIn('Bucket List', response.json)
        self.assert200(response)

    def test_bucket_list_results_when_none_exist(self):
        '''Test for bucketlist results when user has not created any bucket lists'''
        #Delete all bucketlists by user first
        self.BL.query.filter_by(created_by=g.user_id).delete()
        self.database.session.commit()
        response = self.app.get('/bucketlists/', headers=self.header_content_token)
        self.assertEquals(response.json, {'message': 'No Bucket Lists Created'})
        self.assert200(response)
    ...
    def test_creation_of_bucket_list_with_no_name(self):
        ...
        data ={'name': ''}
        response = self.app.post('/bucketlists/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Please provide name for your bucketlist'})
        self.assert400(response)

    def test_creation_of_bucket_list_with_name_that_exists(self):
        '''Test for creating a bucketlist with name that already exists on user list'''
        response = self.app.post('/bucketlists/', data=self.bucket_list_form, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Bucketlist name already exists'})
        self.assert400(response)
    ...
    def test_successful_creation_of_bucket_list(self):
        ...
        response = self.app.post('/bucketlists/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Bucketlist Created Successfully'})
        self.assert200(response)

    def test_update_of_bucket_list_with_no_new_data(self):
        '''Test for update of existing bucket list proving empty data'''
        data = {'name': ''}
        response = self.app.put('/bucketlists/1/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Please provide new Bucket List Name'})
        self.assert200(response)

    def test_update_of_bucket_list_with_name_that_exists(self):
        '''Test for update of existing bucket list proving name that is already in use'''
        #Create a new bucket list to test duplicate names
        data = {'name': 'My New Bucketlist'}
        bucket_list_response = self.app.post('/bucketlists/', data=data, headers=self.header_content_token)
        response = self.app.put('/bucketlists/1/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Name already in use'})
        self.assert200(response)

    def test_update_of_bucket_list_with_non_existent_id(self):
        '''Test for update of non existent bucketlist(No Bucket list with given ID)'''
        data = {'name': 'My New Bucketlist'}
        response = self.app.put('/bucketlists/23/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'No bucketlist with that ID'})
        self.assert200(response)
    ...
    def test_update_of_bucket_that_exists(self):
        ...
        data = {'name': 'My New Bucketlist'}
        response = self.app.put('/bucketlists/1/', data=data, headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'Bucket List updated Successfully'})
        self.assert200(response)

    def test_deletion_of_bucket_list_for_non_existent_id(self):
        '''Test deletion of bucket list with non existent id'''
        response = self.app.delete('/bucketlists/405/', headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'No Bucket List with that ID'})
        self.assert200(response)

    def test_deletion_of_bucket_list_for_existing_id(self):
        '''Test deletion of bucket list with existing id'''
        response = self.app.delete('/bucketlists/1/', headers=self.header_content_token)
        self.assertEqual(response.json, {'message': 'BucketList Deleted Successfully'})
        self.assert200(response)
    ...
    def test_create_token(self):
        ...
        login_response = self.app.post('/auth/login/', data=self.reg_data) #Login User
        self.assertIn('token', login_response.json, msg='Token was not created')
